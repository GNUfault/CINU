...
extern unsigned char _binary_font_bin_size[];

#define font _binary_font_bin_start
...
static void scroll_screen(void) {
    int i;
    unsigned int* src = (unsigned int*)(framebuffer + FONT_HEIGHT * VGA_WIDTH * 3);
    unsigned int* dst = (unsigned int*)framebuffer;
    for (i = 0; i < (VGA_HEIGHT - 1) * FONT_HEIGHT * VGA_WIDTH * 3 / 4; i++) {
        dst[i] = src[i];
    }
    unsigned int* clear = (unsigned int*)(framebuffer + (VGA_HEIGHT - 1) * FONT_HEIGHT * VGA_WIDTH * 3);
...
void printk(const char* str) {
    ...
    while (*str) {
        ...
        for (row = 0; row < FONT_HEIGHT; row++) {
            ...
            unsigned int* pixel = (unsigned int*)(framebuffer + (cursor_y * FONT_HEIGHT + row) * VGA_WIDTH * 3 + cursor_x * 3);
